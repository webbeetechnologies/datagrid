...
import { GestureResponderEvent, StyleSheet, useWindowDimensions, ViewStyle } from 'react-native';
...
import { EventObject, ReversedEventEmitter } from '../utils/reversed-eventemitter';
...
export const _Stage = memo(
    forwardRef(({ width, height, style, children, scrollPositionRef }: StageProps, _ref: any) => {
        ...
        return (
            <Canvas ref={ref} style={canvasStyle} onTouchStart={onTouchStart}>
                <>{children}</>
            </Canvas>
        );
    }),
...
const emitter = new ReversedEventEmitter();
...
export function useTouchEvent(
    ...
    useEffect(() => {
        ...
        emitter.on('touch', handleTouch);

        return () => {
            emitter.off('touch', handleTouch);
        };
